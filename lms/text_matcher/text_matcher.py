...
def checkLog(logfile, textpair):
    ...
    """
    pairs = []
    print('Looking in the log for textpair:' % textpair)
    ...
    # print('Pairs already in log: %s' % pairs)
...
def PDFReader(filename):
    pdfFileObj = open(filename, 'rb')  # creating a pdf file object
    pdfReader = PyPDF2.PdfFileReader(pdfFileObj)  # creating a pdf reader object
    num_pages = pdfReader.numPages  # getting number of pages in pdf file
    full_text = ''
    for pages in range(num_pages):
        pageObj = pdfReader.getPage(pages)  # creating a page object
...
def cli(text1, text2, threshold=3, cutoff=5, ngrams=3, logfile=logfile, verbose=False, stops=False):
    ...
    if stops:
        print('Including stopwords in tokenizing.')

    print('Comparing this/these text(s): %s' % str(texts1))
    print('with this/these text(s): %s' % str(texts2))

    pairs = list(itertools.product(texts1, texts2))

    numPairs = len(pairs)

    print('Comparing %s pairs.' % numPairs)
    # print('List of pairs to compare: %s' % pairs)
    ...
    for index, pair in enumerate(pairs):
        ...
        print('Now comparing pair %s of %s.' % (index + 1, numPairs))
        print('Comparing %s with %s.' % (pair[0], pair[1]))
        ...
        print('Matching completed in %s seconds.' % timeElapsed)
